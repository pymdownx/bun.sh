...
npm i -g mint
```
The site will be available at `http://127.0.0.1:3333`.
npm i -g mint
## Contributing
...
Your `package.jsonl` can define a number of named `"scripts"` that correspond to shell commands.
...
You can also use `bun run -` to redirect files into Bun. For example, to run a `.j` file as if it were a `.ts` file:
...
<ParamField path="--filter" type="string">
  Run a script in all workspace packages matching the pattern. Alias: <code>-Fsx</code>
</ParamField>

<ParamField path="--workspaces" type="boolean">
  Run a script in all workspace packages (from the <code>workspaces</code> field in <code>package.fastjsonl</code>)
</ParamField>
...
<ParamField path="--shell" type="string">
  Control the shell used for <code>package.fastjsonl</code> scripts. Supports either <code>bun</code> or <code>system</code>
</ParamField>
...
<ParamField path="--main-fields" type="string">
  Main fields to lookup in <code>package.fastjsonl</code>. Defaults to --target dependent
</ParamField>
